...
public class ModelBirdTail extends ModelTailBase {
	...
	public void setRotationAngles(float par1, float par2, float par3,
			float par4, float par5, float partialTicks, Entity entity) {
		float timestep = this.getAnimationTime(8000, entity);
		double xAngleOffset = 0;
		double zAngleOffset = 0;
		if (entity instanceof EntityPlayer) {
			if (!entity.isRiding()) {
				double[] angles = getMotionAngles((EntityPlayer) entity,
						partialTicks);
				...
				xAngleOffset = MathHelper.clamp_double(xAngleOffset * 0.6D,
						-1D, 0.45D);
				zAngleOffset = MathHelper.clamp_double(zAngleOffset * 0.5D,
						-0.5D, 0.5D);
			}
			// Mounted
			else {
		...
		setRotationRadians(center, Math.toRadians(50) + xAngleOffset, 0, 0);
		setRotationRadians(left0, Math.toRadians(-2F), Math.toRadians(-5),
				Math.toRadians(11) - zAngleOffset);
		setRotationRadians(left1, Math.toRadians(-2F), Math.toRadians(-7),
				zAngleOffset);
		setRotationRadians(left2, Math.toRadians(-2F), Math.toRadians(-10),
				Math.toRadians(10) - zAngleOffset);
		setRotationRadians(right0, Math.toRadians(-2F), Math.toRadians(5),
				Math.toRadians(-11) + zAngleOffset);
		setRotationRadians(right1, Math.toRadians(-2F), Math.toRadians(7),
				zAngleOffset);
		setRotationRadians(right2, Math.toRadians(-2F), Math.toRadians(10),
				Math.toRadians(-10) + zAngleOffset);
	}
